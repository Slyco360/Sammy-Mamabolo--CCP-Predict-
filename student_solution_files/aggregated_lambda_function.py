...
def lambda_handler(event, context):
    ...
    # --- Insert your code here ---
    # below the docstring is a test case you can comment out the body_enc and dec_dict and remove the docstring to test the lambda handler 
    ...
    # Get list of phrases in numpy arra
    phrase = []
    for i in range(0, len(key_phrases['KeyPhrases'])-1):
        phrase = np.append(phrase, key_phrases['KeyPhrases'][i]['Text'])
    
    critical_phrase_list = ['CV', 'data' , 'science' , 'projects', 'articles', 'Medium' , 'publish', 'blog']
    name = dec_dict['name']
