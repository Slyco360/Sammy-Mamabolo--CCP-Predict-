...
def generate_unique_rid(table):
    while True:
        rid = random.randint(1, 1000000000)
        # Check if the generated rid already exists in the table
        response = table.get_item(Key={'ResponsesID': rid})
...
def lambda_handler(event, context):
    ...
    table = dynamodb.Table('2401FT-MOKMAM-portfolio-data-table')
   
    rid = generate_unique_rid(table)
    ...
    # Amazon Comprehend Functionality
    ...
    email_text = email_response(dec_dict['name'], ['CV', 'resume', 'Resume', 'projects', 'portfolio', 'data'], phrase, sentiment)
